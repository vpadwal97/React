...
const SearchNotifier = () => {
  ...
  const sn =TooltipMessages.global.communication.notifierdefination.SearchNotifier;
  ...
  const columns = useMemo(() => [
    {
      accessorKey: 'currencyCode', header: 'Details', size: 150, Cell: ({ row }) => (<span style={{ fontWeight: "bold", color: "#2b2c2c", cursor: "pointer" }}>Currency Description:{row.original.currencyName}
        <br />Currency Code:{row.original.currencyCode} <br />Currency Symbol:{row.original.currencySymbol}</span>)
    },], []);

  const handleClickCreate = () => {
    setAction("create")
    const timer = setTimeout(() => {
      navigate("/communication/NotifierDefinition")
  ...
  const getEditApi = async (id) => {
    try {
      const response = await axios.get(`api/mailerDefinitionMaster/${id}`);
  ...
  const getSearch = async (newPage, newRowsPerPage) => {
    ...
    const response = await axios
      .post("api/mailerDefinitionMaster/search-by-criteria", {
                  ...
                  <div className="col-md-4 mt-3">
                    <div class="select_shadow form-floating">
                      ...
                      <label for="input" style={{ fontSize: "16px", }}>Notify Type
                        <span><IoIosInformationCircleCustom tooltipMessage={sn.NotifyType} /></span>
                      </label>
                    </div>
                  </div>
                  <div className="col-md-4 mt-3"><FormGroup tooltipMessage={sn.name} type="text" label="Name" placeholder="" /></div>
                  <div className="col-md-4 mt-3"><FormGroupRadio tooltipMessage={sn.isactive} title={ButtonLabels.Active_Status} value={isactive} onChange={() => setIsActive(!isactive)} /></div>
=======
                    ...
                    <Tooltip arrow placement="left" title="Edit">
                      <a>
                        <button className="border-0 rounded-2 p-2" style={{ background: "#349F01", color: "#fff" }} onClick={() => {
                          getEditApi(row.original.currencyConvRfnum);
                          handleClickEdit();
                        }}>
                          <Edit />
                        </button>
